...
export default function ProfilePage() {
  return (
    <main className="mx-auto w-full max-w-xl px-6 py-16">
      ...
      <h1 className="text-brand-card mt-6 text-2xl font-semibold tracking-tight">Your profile</h1>
      ...
      <section className="mt-12">
        <h2 className="text-brand-card text-lg font-semibold tracking-tight">Change password</h2>
        <p className="text-brand-muted mt-2 mb-7 text-sm leading-6">
...
async function AccountDetails() {
  ...
  return (
    <dl className="text-brand-muted mt-6 space-y-2 text-sm">
...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex gap-2">
      ...
      <dd className="text-brand-card">{children}</dd>
    </div>
  );
}
